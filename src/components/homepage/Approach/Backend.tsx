...
import BackgroundPNG from "images/homepage/approach/backend-background.webp"
import BackendSVG from "images/homepage/approach/backend.svg"
...
export default function Backend({ timeline }: BackendProps) {
  ...
  return (
    <Wrapper>
      ...
      />
      <BG ref={svg} />
      <Text ref={text} $shift={-3.4}>{t.skills.backend}</Text>
      <BeforeText ref={beforeText}>{t.skills.backendShort}</BeforeText>
    </Wrapper>
  )
}

const BG = styled(BackendSVG)`
  position: absolute;
  top: 0;
  left: 0;
  width: 100%;
