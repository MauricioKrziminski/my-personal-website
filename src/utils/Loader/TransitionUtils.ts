...
import { routerNavigate } from "utils/nextRouter"
...
export const registerTransition = (
  ...
  allTransitions[name] = {
    inAnimation: [
      ...
      {
        callback: inAnimation,
        duration: inDuration,
        revert: details.revert ?? true,
      },
    ],
    outAnimation: [
      ...previous.outAnimation,
      {
        callback: outAnimation,
...
export const loadPage = async (
  ...
  const entranceDuration = enterAnimations.reduce((duration, animation) => {
    ...
    if (!animation.revert) callback()
    else animationContext.add(callback)
  ...
  if (pendingTransition) {
    ...
    loadPage(
      ...
      pendingTransition.transition ?? "none"
    ).catch((e: string) => {
      throw new Error(e)
    })
